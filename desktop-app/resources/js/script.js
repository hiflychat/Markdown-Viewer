...
`;
  ...
  async function openDiagramModal() {
    ...
    // ABC Notation thumbnails use local SVGs so tiny card previews stay legible.
    function createAbcStaff(y, x1 = 18, x2 = 182, gap = 6) {
      return [0, 1, 2, 3, 4].map(i => `<line class="abc-thumb-staff" x1="${x1}" y1="${y + i * gap}" x2="${x2}" y2="${y + i * gap}" stroke="#111827" stroke-width="1.15" stroke-linecap="round"/>`).join('');
    }

    function createAbcNote(cx, cy, stemUp = true) {
      const stemX = stemUp ? cx + 5 : cx - 5;
      const stemEnd = stemUp ? cy - 34 : cy + 34;
      return `<g class="abc-thumb-note"><ellipse cx="${cx}" cy="${cy}" rx="7" ry="5" transform="rotate(-18 ${cx} ${cy})" fill="#111827"/><line x1="${stemX}" y1="${cy}" x2="${stemX}" y2="${stemEnd}" stroke="#111827" stroke-width="3" stroke-linecap="round"/></g>`;
    }

    function createAbcThumb(inner, viewBox = '0 0 200 96') {
      return `<svg class="abc-thumb-svg" viewBox="${viewBox}" width="100%" height="100%" aria-hidden="true">${inner}</svg>`;
    }

    const svgAbcMelody = createAbcThumb(`${createAbcStaff(32)}${createAbcNote(54, 58)}${createAbcNote(92, 50)}${createAbcNote(130, 44)}<path class="abc-thumb-beam" d="M59 24 L135 10" stroke="#111827" stroke-width="3" stroke-linecap="round" fill="none"/>`);
    const svgAbcDuet = createAbcThumb(`${createAbcStaff(14)}${createAbcStaff(58)}${createAbcNote(54, 40)}${createAbcNote(108, 78, false)}<path class="abc-thumb-brace" d="M28 13 C18 24, 18 48, 28 58 C18 68, 18 86, 28 95" stroke="#111827" stroke-width="1.8" stroke-linecap="round" fill="none"/>`);
    const svgAbcLyric = createAbcThumb(`${createAbcStaff(26)}${createAbcNote(52, 52)}${createAbcNote(92, 46)}${createAbcNote(132, 52)}<text class="abc-thumb-lyric" x="42" y="78" fill="#111827">Do</text><text class="abc-thumb-lyric" x="84" y="78" fill="#111827">Re</text><text class="abc-thumb-lyric" x="124" y="78" fill="#111827">Mi</text>`);
    const svgAbcChords = createAbcThumb(`<text class="abc-thumb-chord" x="50" y="22" fill="#2563eb">C</text><text class="abc-thumb-chord" x="108" y="22" fill="#2563eb">G7</text>${createAbcStaff(34)}${createAbcNote(58, 54)}${createAbcNote(120, 48)}`);
    ...
    // ABC Notation additional thumbnails
    const svgAbcPolyphony = createAbcThumb(`${createAbcStaff(30)}${createAbcNote(62, 45)}${createAbcNote(62, 60, false)}${createAbcNote(122, 39)}${createAbcNote(122, 66, false)}<path class="abc-thumb-slur" d="M50 70 C76 84, 112 84, 136 70" stroke="#111827" stroke-width="1.8" stroke-linecap="round" fill="none"/>`);
    const svgAbcKeySignature = createAbcThumb(`${createAbcStaff(30)}<text class="abc-thumb-clef" x="28" y="64" fill="#111827">𝄞</text><text class="abc-thumb-sharp" x="56" y="42" fill="#111827">♯</text><text class="abc-thumb-sharp" x="68" y="54" fill="#111827">♯</text>${createAbcNote(110, 52)}${createAbcNote(152, 58)}`);
