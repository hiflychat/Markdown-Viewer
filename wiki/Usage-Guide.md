...
5. After feedback is attached, select its review icon and count—or the reviewed content itself—to open the saved item in the panel.
6. Use **Resolve** or **Reopen** to manage a thread. The panel toolbar can copy a Markdown summary, resolve every open item, or delete all feedback after confirmation. Deleting an individual item also uses an in-app confirmation dialog.
