...
- **Comments & Suggestions** provides a per-document review layer. Review pins attach comments or suggestions to rendered YAML frontmatter tables, headings, paragraphs, code blocks, and diagrams while the Markdown stays read-only. A plus opens the composer; a review count or reviewed block opens its existing feedback and the composer for adding another item to the same block. Threads can be edited, resolved, reopened, deleted, or copied as a Markdown summary. The interface uses a desktop side panel, a tablet drawer, and a touch-friendly mobile bottom sheet. Review data stays with the local tab workspace, is excluded from Share Snapshot, and is relayed only while an existing Live Share room is active.
